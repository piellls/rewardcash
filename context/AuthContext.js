...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const checkVPN = async () => {
      try {
        ...
        const simulateVPN = urlParams.get('simulate_vpn');
        
        const res = await fetch(`/api/check-ip${simulateVPN === '1' ? '?simulate=1' : ''}`);
  ...
  // Fullscreen VPN/Proxy connection blocking screen
  if (vpnDetected) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#050811] p-4 text-center">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-20" style={{
          backgroundImage: `
            radial-gradient(at 50% 0%, rgba(245, 158, 11, 0.15) 0px, transparent 60%),
        ...
        <div className="relative w-full max-w-md rounded-2xl border border-amber-500/20 bg-dark-card/95 p-8 shadow-2xl backdrop-blur-md space-y-6">
          {/* Top warning line */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 via-primary to-amber-500" />
          
          <div className="inline-flex rounded-full bg-amber-500/10 p-4 text-amber-500 animate-bounce">
            <Globe className="h-10 w-10" />
          </div>

          <div className="space-y-2">
            <h1 className="text-2xl font-black text-white tracking-tight uppercase">Connection Blocked</h1>
            <p className="text-xs text-amber-400 font-bold uppercase tracking-widest animate-pulse">VPN or Proxy Detected</p>
          </div>

          <div className="rounded-xl bg-zinc-950/80 p-4 border border-dark-border/60 text-left space-y-2.5">
            <p className="text-xs text-zinc-300 leading-relaxed">
              RewardCash strictly prohibits the use of VPNs, web proxies, or hosting network routers:
            </p>
            
            {vpnDetails && (
              <div className="space-y-1.5 text-xs bg-amber-950/20 border border-amber-900/40 p-3 rounded-lg font-mono text-zinc-400">
                <div className="flex justify-between">
                  <span className="text-zinc-550">Client IP:</span>
                  <span className="text-white font-bold">{vpnDetails.ip}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-550">Detected Country:</span>
                  <span className="text-white font-bold">{vpnDetails.country}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-550">Provider:</span>
                  <span className="text-white font-bold truncate max-w-[180px]">{vpnDetails.provider}</span>
                </div>
              </div>
            )}
            
            <p className="text-[10.5px] text-zinc-550 leading-relaxed pt-1">
              To guarantee high offer quality for our advertisers and protect reward fairness, please disconnect from any proxy tunnel and reload the page.
            </p>
          </div>
