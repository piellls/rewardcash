    ...
    <>
      ...
      <header className="fixed top-0 right-0 left-0 md:left-64 z-30 h-16 glass-nav flex items-center justify-between px-4 sm:px-6 lg:px-8 border-b border-dark-border">
        ...
        <div className="flex items-center md:hidden">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="rounded-lg bg-gradient-to-tr from-primary to-secondary p-1.5 shadow-[0_0_12px_rgba(56,189,248,0.35)]">
        ...
        <div className="hidden md:flex items-center gap-3 bg-zinc-950/40 border border-dark-border/60 rounded-xl px-4 py-1.5 max-w-sm w-full group relative cursor-pointer transition-colors hover:border-primary/20">
          {user ? (
            <>
              ...
              <div className="absolute top-11 left-0 right-0 p-3 bg-dark-card border border-dark-border rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 text-xs text-zinc-400 space-y-1.5">
                <p className="font-bold text-white text-center border-b border-dark-border/40 pb-1 mb-1">XP Level Details</p>
                ...
                <p className="text-[10px] text-zinc-550 leading-relaxed pt-1">Earn more coins by completing offers to level up. Higher levels show your dominance f the rankings!</p>
              </div>
            </>
          ) : (
        ...
        <div className="flex items-center gap-4">
          {loading ? (
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              {/* Coin Display */}
              <div className="flex items-center gap-2 rounded-xl bg-zinc-950 border border-dark-border px-3 py-1.5 shadow-[inset_0_1px_2px_rgba(0,0,0,0.6)]">
              ...
              >
                <img 
                  ...
                  alt={user.username}
                  className="h-8 w-8 rounded-full border border-dark-border group-hover:border-primary/40 transition-colors object-cover"
                />
              ...
              <button 
                onClick={logout}
                className="rounded-xl p-2 text-zinc-500 hover:bg-zinc-900 hover:text-white active:scale-95 transition-all"
                title="Log Out"
              >
      ...
      <aside className="hidden md:flex fixed top-0 left-0 bottom-0 w-64 bg-dark-card border-r border-dark-border flex-col z-40">
        ...
        <div className="h-16 border-b border-dark-border/40 flex items-center px-6 shrink-0 justify-between">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="rounded-lg bg-gradient-to-tr from-secondary to-primary p-1.5 shadow-[0_0_12px_rgba(56,189,248,0.3)] transition-transform group-hover:scale-105">
              <Coins className="h-5 w-5 text-black" />
            </div>
        ...
        <div className="p-4 border-t border-dark-border/40 shrink-0 space-y-2">
          ...
          <div className="text-[10px] text-zinc-550 text-center py-1 font-semibold uppercase tracking-wider">
            RewardCash v1.3.0 • EarnLab Style
          </div>
        </div>
      </aside>
